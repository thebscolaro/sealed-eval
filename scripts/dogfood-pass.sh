#!/usr/bin/env bash
# Dogfood: healthy subject-demo + orders fixture -> expect grade exit 0
set -euo pipefail
...
PORT="${SE_SUBJECT_PORT:-8080}"
SUITE="${SE_SUITE:-orders-v1}"
...
set -e
if [[ "$EC" -ne 0 ]]; then
  echo "dogfood-pass: expected exit 0, got $EC" >&2
  exit 1
fi
echo "dogfood-pass: OK"
